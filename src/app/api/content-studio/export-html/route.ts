...
export async function POST(request: NextRequest) {
  try {
    ...
    const { blocks, template, metadata, format = "web" } = body;
    ...
    const html =
      format === "email"
        ? generateEmailHTMLFromBlocks(blocks, template, metadata)
        : generateHTMLFromBlocks(blocks, template, metadata);
...
/**
 * Generate Mailchimp-compatible email HTML from content blocks
 * Follows 2025 Mailchimp guidelines for custom HTML
 *
...
function generateEmailHTMLFromBlocks(
  ...
  // Create Mailchimp-compatible email HTML
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f4f4f4;">
        <tr>
            <td align="center" style="padding: ${headerHTML.trim() ? "0" : "20px"} 0 20px 0;">
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="margin: 20px auto 0; max-width: 600px;">
                    <tr>
                        <td align="center" style="padding: 20px; font-family: Arial, sans-serif; font-size: 12px; line-height: 18px; color: #666666;">
                            <p style="margin: 0;">
                                Generated by Motive Archive Manager Content Studio<br>
                                Exported: ${exportedAt}
                            </p>
                            <!-- Add Mailchimp merge tags for compliance -->
...
function generateEmailBlockHTML(block: ContentBlock): string {
  switch (block.type) {
    case "text": {
      ...
      // Email-safe inline styles based on element type
      let inlineStyles = "";
      switch (element) {
        case "h1":
          inlineStyles =
            'style="margin: 0 0 20px 0; font-family: Arial, sans-serif; font-size: 28px; line-height: 34px; font-weight: bold; color: #333333;" class="dark-text mobile-font-size"';
          break;
        case "h2":
          inlineStyles =
            'style="margin: 0 0 18px 0; font-family: Arial, sans-serif; font-size: 32px; line-height: 38px; font-weight: bold; color: #333333;" class="dark-text mobile-font-size"';
          break;
        case "h3":
          inlineStyles =
            'style="margin: 0 0 16px 0; font-family: Arial, sans-serif; font-size: 20px; line-height: 26px; font-weight: bold; color: #333333;" class="dark-text mobile-font-size"';
          break;
        default: // p
          inlineStyles =
            'style="margin: 0 0 16px 0; font-family: Arial, sans-serif; font-size: 16px; line-height: 24px; color: #333333;" class="dark-text"';
      }
      ...
      return `<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin-bottom: 20px;">
        <tr>
            <td mc:edit="text_block_${block.id}">
                <${element} ${inlineStyles}>${finalContent}</${element}>
            </td>
        </tr>
    </table>`;
    }
