...
export interface CarEntryFormRef {
  populateForm: (data: Partial<CarFormData>) => void;
}
...
const CarEntryForm = forwardRef<CarEntryFormRef, CarEntryFormProps>(
  ({ onSubmit, isSubmitting }, ref) => {
    ...
    const [
      ...
      setIsDecodingVinWithoutCorrections,
    ] = useState(false);

    // Expose the populateForm method to parent components
    useImperativeHandle(
      ref,
      () => ({
        populateForm: (data: Partial<CarFormData>) => {
          setFormData((prevData) => {
            // Merge the incoming data with existing form data, preserving structure
            const mergedData = { ...prevData };

            // Handle basic fields
            Object.keys(data).forEach((key) => {
              if (key === "engine" && data.engine) {
                mergedData.engine = {
                  ...prevData.engine,
                  ...data.engine,
                  displacement:
                    data.engine.displacement || prevData.engine.displacement,
                  power: data.engine.power || prevData.engine.power,
                  torque: data.engine.torque || prevData.engine.torque,
                  features: data.engine.features || prevData.engine.features,
                };
              } else if (key === "dimensions" && data.dimensions) {
                mergedData.dimensions = {
                  ...prevData.dimensions,
                  ...data.dimensions,
                };
              } else if (key === "manufacturing" && data.manufacturing) {
                mergedData.manufacturing = {
                  ...prevData.manufacturing,
                  ...data.manufacturing,
                  plant: {
                    ...prevData.manufacturing?.plant,
                    ...data.manufacturing.plant,
                  },
                };
              } else if (key === "price" && data.price) {
                mergedData.price = {
                  ...prevData.price,
                  ...data.price,
                };
              } else if (data[key as keyof CarFormData] !== undefined) {
                (mergedData as any)[key] = data[key as keyof CarFormData];
              }
            });
    ...
    const handleVinError = (data: VINResponse, toastId: string) => {
      ...
      if (
        errorCodes.includes("14") ||
        (errorCodes.includes("1") && data.make)
      ) {
        toast.warning(
          <div className="space-y-2">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange("interior_color", e.target.value)
                  }
                  className={inputClasses}
                />
              </div>
        ...
        <div className="rounded-lg border border-border-primary divide-y divide-border-primary p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <MeasurementInputWithUnit
                value={{
                  value: formData.engine.torque["lb-ft"],
                  unit: "lb-ft",
                }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleChange("doors", parseInt(e.target.value))
                }
                className={inputClasses}
              />
            </div>
...
CarEntryForm.displayName = "CarEntryForm";
